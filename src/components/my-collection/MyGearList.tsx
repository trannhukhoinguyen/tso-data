...
export default function MyGearList() {
    const gears = useStore(myGears);

    const handleAddToCart = (gear: any) => {
        const cartGear = {
            id: gear.id,
            name: gear.name,
            price: gear.price,
        ...
        const result = addToCart(cartGear, 1);
        if (result.success) {
    ...
    const handleRemove = (id: string, name: string) => {
        const result = removeFromMyGears(id);
        if (result.success) {
            toast.info(`${name} removed from gears`);
        }
    };
    ...
    if (gears.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[50vh] text-center p-4">
                ...
                <h2 className="text-2xl font-bold mb-2">No gears yet</h2>
                <p className="text-base-content/70 mb-6 max-w-md">
                    Click the heart icon on any gear to save it here for later.
                </p>
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {gears.map((gear) => (
                <div key={gear.id} className="card bg-base-100 shadow-sm hover:shadow-xl transition-shadow duration-300 border border-base-200 overflow-hidden relative group">
                    <button
                        onClick={() => handleRemove(gear.id, gear.name)}
                    ...
                    <a href={`/gear/${gear.slug}`} className="block relative aspect-4/5 bg-gray-100 overflow-hidden">
                        <img
                            src={gear.image}
                            alt={gear.name}
                    ...
                    <div className="p-4">
                        <h3 className="font-bold text-lg mb-1 truncate">
                            <a href={`/gear/${gear.slug}`} className="hover:text-primary transition-colors">
                                {gear.name}
                            </a>
                        </h3>
                        ...
                        <div className="flex items-center justify-between mt-4">
                            <span className="text-xl font-bold text-primary">
                                ${gear.price.toLocaleString("en-US")}
                            </span>
                            <button
